...
const HourlyForecast: React.FC<{ countryName: string }> = ({ query }) => {
  const [hourForecast, setHourForecast] = useState<ForecastData | null>(null);
  ...
  return (
    <div className="max-w-[51%] w-full backdrop-blur-sm bg-gray-500/40 p-3 h-[400px] overflow-y-auto rounded-lg">
      <h2 className="text-[32px] font-semibold  text-center mb-3 text-[#fff]">Hourly Forecast</h2>
      {hourForecast?.forecast.forecastday.map((item) =>
        item.hour.map((it, index) => (
          ...
          >
            <p className="col-span-1">{new Date(it.time).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
            <figure className="col-span-1 flex justify-center">
              <img src={`https:${it.condition.icon}`} alt={it.condition.text} className="w-8 h-8" />
            </figure>
